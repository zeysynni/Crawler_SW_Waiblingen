...
from enrich import extract_accordion_qas, extract_pdf_files
...
def test_extract_accordion_qas_gets_every_pair():
    qas = extract_accordion_qas(ACCORDION_HTML)
    assert len(qas) == 2
...
def test_extract_accordion_qas_skips_empty_or_echoed():
    html = """
    <div class="accordion-item">
    ...
    """
    assert extract_accordion_qas(html) == []


def test_extract_accordion_qas_empty_when_no_accordion():
    assert extract_accordion_qas("<p>no accordions here</p>") == []
